...
def ComputeError(Yp, Y, r_list=[2], sh=6, n_fold=10, seed=131):
    # Bootstrap evaluation
    metric_list = np.zeros((n_fold, 1+len(r_list)))
    random_indices = np.arange(len(Y))
    np.random.RandomState(seed).shuffle(random_indices)
    for i in range(n_fold):
        indicies = random_indices[i*len(Y)//n_fold:(i+1)*len(Y)//n_fold]
        Yp_fold = Yp[indicies]
        Y_fold = Y[indicies]
